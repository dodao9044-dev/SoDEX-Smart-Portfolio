...
export async function handler(event) {
  ...
  try {
    try {
      ...
      return json(200, soso);
    } catch {
      const fb = await fallback(resource);
      return json(200, { ok: true, source: fb.source, resource, data: fb.data, protected: true, fallback: true });
    }
  } catch {
    ...
    return json(200, { ok: true, source: 'local-resilience', resource, data, protected: true, fallback: true });
  }
}
