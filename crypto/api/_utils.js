...
export function endpointBase(market = 'perps') {
  ...
  if (market === 'spot') {
    return env('SODEX_SPOT_REST_BASE', network === 'testnet' ? 'https://testnet-gw.sodex.dev/api/v1/spot' : 'https://mainnet-gw.sodex.dev/api/v1/spot');
  }
...
export async function signSodexAction({ type, params, market }) {
  const { apiKeyName, privateKey, missing } = requireTradingEnv();
  if (missing.length) {
