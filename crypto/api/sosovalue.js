...
export default async function handler(req) {
  ...
  try {
    ...
    return json({ ...soso, live: true, fallback: false });
  } catch (sosoError) {
    if (strict) {
      return json({ ok: false, source: 'sosovalue', resource, error: sosoError.message, hint: 'Check SOSOVALUE_API_KEY / SOSOVALUE_*_PATH in Vercel Environment Variables.' }, sosoError.status || 502);
    }
    try {
      const fb = await fallback(resource);
      return json({ ok: true, source: fb.source, resource, data: fb.data, live: true, fallback: true, upstreamError: debug ? sosoError.message : undefined });
    } catch (liveError) {
      return json({ ok: false, source: 'none', resource, data: [], live: false, fallback: false, error: liveError.message, upstreamError: debug ? sosoError.message : undefined }, liveError.status || 502);
    }
  }
}
