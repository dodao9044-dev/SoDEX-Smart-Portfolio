...
function App() {
  ...
  return <div className="app">
    <aside><div className="logo">S</div><b>ValuePilot</b><small>Desk</small>{navItems.map(([id,label])=><button key={id} onClick={()=>setView(id)} className={view===id?'active':''}>{label}</button>)}<div className="bottom"><button onClick={()=>setView('analysis')}>AI cockpit</button><button onClick={()=>setView('execution')}>Trade panel</button></div></aside>
    <main>
