...
function clientFallbackDatasets() {
  return { market: [], ssi: [], etf: [], news: [] };
}

function ensureRows(next) {
  return {
    market: next.market?.length ? next.market : [],
    ssi: next.ssi?.length ? next.ssi : [],
    etf: next.etf?.length ? next.etf : [],
    news: next.news?.length ? next.news : []
  };
}
...
function App() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      setSourceMap({ market: `error: ${error.message}`, ssi: `error: ${error.message}`, etf: `error: ${error.message}`, news: `error: ${error.message}` });
      setSelectedRow(null);
    } finally {
      setLoading(false);
    }
